module demo::bytecode {
    ...
    struct S has key {
        nonce: table::Table<address, u256>,
        contracts: table::Table<vector<u8>, T>
    }

    entry fun init_module(account: &signer) {
        debug::print(&address_from_bytes(keccak256(u256_to_data((now_microseconds() as u256)))));
        move_to(account, S {
            nonce: table::new<address, u256>(),
            contracts: table::new<vector<u8>, T>()
        });
    }

    public entry fun deploy(account: &signer, runtime: vector<u8>, construct: vector<u8>): vector<u8> acquires S {
        let account_addr = signer::address_of(account);
        let global = borrow_global_mut<S>(@demo);
        let nonce = 1;
        if(!table::contains(&global.nonce, account_addr)) {
            table::add(&mut global.nonce, account_addr, 1);
        } else {
            let nonce_table = table::borrow_mut(&mut global.nonce, account_addr);
        ...
        let contract_addr = keccak256(bytes);
        assert!(!table::contains(&global.contracts, contract_addr), CONTRACT_DEPLOYED);
        table::add(&mut global.contracts, contract_addr, T {
    ...
    fun create(contract_addr: vector<u8>, )

    public entry fun call(contract_addr: vector<u8>, data: vector<u8>) acquires T  {
        ...
        while (i < len) {
            ...
            //codecopy
                ...
                while (d_pos < end) {
                    ...
                    simple_map::upsert(memory, m_pos, data_to_u256(bytes, 0, 32));
                    d_pos = d_pos + 32;
                    m_pos = m_pos + 32;
                };
                i = i + 1
    ...
    fun read_bytes(memory: &mut simple_map::SimpleMap<u256, u256>, pos: u256, end: u256): vector<u8> {
        let bytes = vector::empty<u8>();
        while(pos < end) {
            if(simple_map::contains_key(memory, &pos)) {
                let value = simple_map::borrow(memory, &pos);
                vector::append(&mut bytes, u256_to_data(*value));
            }
            else {
                vector::append(&mut bytes, u256_to_data(0));
            };
            pos = pos + 32;
        };
