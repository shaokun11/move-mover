module demo::evmcontract {
    ...
    struct S has key {
        nonce: simple_map::SimpleMap<vector<u8>, u256>,
    ...
    entry fun init_module(account: &signer) {
        move_to(account, S {
            nonce: simple_map::create<vector<u8>, u256>(),
    ...
    fun create(_account: &signer, sender: vector<u8>, construct: vector<u8>): vector<u8> acquires S {
        let global = borrow_global_mut<S>(@demo);
        let nonce = 1;
        if(!simple_map::contains_key(&global.nonce, &sender)) {
            simple_map::add(&mut global.nonce, sender, 1);
        } else {
            let nonce_table = simple_map::borrow_mut(&mut global.nonce, &sender);
    ...
    public entry fun call(account: &signer, sender: vector<u8>, contract_addr: vector<u8>, data: vector<u8>) acquires S {
        // todo verify caller
        let _account_addr = signer::address_of(account);
        let global = borrow_global_mut<S>(@demo);
        assert!(vector::length(&sender) == 32, INVALID_SENDER);
    ...
    public fun view(sender:vector<u8>, contract_addr: vector<u8>, data: vector<u8>): vector<u8> acquires S {
        let global = borrow_global_mut<S>(@demo);
        let contract = simple_map::borrow<vector<u8>, T>(&global.contracts, &contract_addr);
        run(global, sender, copy contract_addr, contract.runtime, data, true)
    }
    ...
    fun run(global: &mut S, sender: vector<u8>, contract_addr: vector<u8>, code: vector<u8>, data: vector<u8>, readOnly: bool): vector<u8> {
        ...
        while (i < len) {
            ...
            //RETURNDATASIZE
                ...
                vector::push_back(stack, (now_microseconds() as u256) / 1000);
                i = i + 1;
            }
            //chainid
            else if(opcode == 0x46) {
            ...
            //call 0xf1 static call 0xfa
                ...
                let _msg_value = if(opcode == 0xf1) vector::pop_back(stack) else 0;
                let m_pos = vector::pop_back(stack);
            ...
            //create2
                ...
                let runtime = run(global, copy contract_addr, new_contract_addr, new_codes, x"", false);
    ...
    fun testUniswap() acquires S {
        ...
        let usdt_addr = create(&user, sender, init_code);
        ...
        let factory_addr = create(&user, sender, factory_code);
        ...
        vector::append(&mut router_code, to_32bit(factory_addr));
        let router_addr = create(&user, sender, router_code);
        ...
        call(&user, sender, usdc_addr, approve_usdc_params);
        ...
        call(&user, sender, usdt_addr, approve_usdt_params);
        ...
        call(&user, sender, usdt_addr, mint_usdt_params);
        ...
        call(&user, sender, router_addr, add_liquidity_params);
        ...
        call(&user, sender, router_addr, swap_params);
